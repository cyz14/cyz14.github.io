...
数学家们或者几何学家们常问的问题就是，什么数据可以决定一个**凸多面体、微分形式、度量**的存在，条件是什么？

在 [Convex Polyhedra](/blog/2026/Convex-Polyhedra-Alexandrov/) 的介绍中我们考虑过三个边长满足三角不等式时确定一个欧氏三角形（三个测地线长度确定一个球面、双曲测地三角形），三个和为180的角度确定一组欧氏空间的相似三角形，给定法向量和面积确定凸多面体（欧氏、球面、双曲）的 Minkowski 问题，背后是实的 Monge-Ampère equation。Calabi猜想是，给定拓扑条件和曲率条件，Kähler 流形的度量能否确定？

这是一种压缩观点吗？还是一种关于本质、事物之间深刻联系的观点？
